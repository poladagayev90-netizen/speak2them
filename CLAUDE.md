...
- **Trial gate** (`isTrialExpired`, functions/index.js): trust only rules-protected fields (`isPremium`, `subscriptionPlan`, `cohortStatus`, `freeAccessUntil`). NEVER trust the client-writable `mode`. Trial = 30 days (`TRIAL_DAYS`, changed from 2 in `644a2df8`). Post-trial monetization is undecided — cohort course is the only paid path today; a minute-credit system is being considered but deliberately deferred until there's real usage data.

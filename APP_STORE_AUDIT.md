...
| Bloker | Status | Qeyd |
|---|--------|------|
| B1 Native push | ✅ **Həll olundu** (commit `c5558ed`) | Firebase-də Android tətbiqi yaradıldı (əvvəllər yalnız WEB vardı — `google-services.json`-un heç vaxt mövcud olmamasının səbəbi bu idi), `@capacitor/push-notifications` + `POST_NOTIFICATIONS` əlavə edildi, `src/nativePush.js` tokeni `users/{uid}/fcmTokens`-ə `platform:'android'` ilə yazır. **Əsas tapıntı:** göndərən yalnız `data` yollayırdı — web SW bunu özü göstərir, amma Android arxa fonda səssiz saxlayır (tray-də heç nə). `pushTokens.js` indi platformaya görə qruplaşdırır: Android → `notification` bloku, web → data-only (ikiqat göstərilməsin). ⚠️ Çatdırılma real cihazda (internal testing) yoxlanılmalıdır. |

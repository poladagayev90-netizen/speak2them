  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Header */}
        <div style={{
          background: 'var(--accent)', padding: '30px 20px',
          textAlign: 'center', color: 'var(--text-on-accent)'
        }}>
        ...
        <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '60vh', overflowY: 'auto' }}>
          ...
          {/* Pro Monthly */}
          <div style={{
            border: '2px solid var(--accent)', borderRadius: '16px', padding: '16px',
            background: 'var(--bg-secondary)', position: 'relative'
          }}>
            <div style={{
              position: 'absolute', top: '-12px', right: '16px', background: 'var(--accent)',
              color: 'var(--text-on-accent)', padding: '4px 12px', borderRadius: '12px', fontSize: '11px', fontWeight: 700
            }}>MOST POPULAR</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: '18px', color: 'var(--text-primary)' }}>Pro Paket</h3>
                <p style={{ margin: 0, fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)' }}>Billed monthly</p>
              </div>
              <div style={{ textAlign: 'right' }}>
                <span style={{ fontSize: '22px', fontWeight: 800, color: 'var(--text-primary)' }}>14.99 ₼</span>
                <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)' }}>/ay</span>
              </div>
            </div>
            <ul style={{ padding: 0, margin: '0 0 16px 0', listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <li style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)' }}>
                <Star size={16} color="var(--accent)" /> Unlimited calls and activities
              </li>
              <li style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)' }}>
                <Zap size={16} color="var(--accent)" /> Unlimited AI speech analysis
              </li>
              <li style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)' }}>
                <ShieldCheck size={16} color="var(--accent)" /> Full AI course mode and reports
              </li>
            </ul>
            <button onClick={() => handleUpgrade('Pro Monthly')} style={{
              width: '100%', padding: '12px', background: 'var(--accent)', color: 'var(--text-on-accent)',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <div>
                ...
                <p style={{ margin: 0, fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)' }}>Billed yearly</p>
              </div>
              <div style={{ textAlign: 'right' }}>
                <span style={{ fontSize: '22px', fontWeight: 800, color: 'var(--text-primary)' }}>99.99 ₼</span>
                <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)' }}>/il</span>
              </div>
            </div>

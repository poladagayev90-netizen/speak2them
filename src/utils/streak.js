...
export function streakTier(count) {
  if (count >= 30) {
    return { key: 'legend', title: 'Legend', message: 'A full month. Unstoppable.', accent: 'var(--accent)', effect: 'crown' };
  }
  if (count >= 14) {
    return { key: 'diamond', title: 'Outstanding', message: 'Two weeks. You are in the habit now.', accent: '#22d3ee', effect: 'crown' };
  }
  if (count >= 7) {
    return { key: 'fire', title: 'You are a legend', message: 'A full week', accent: 'var(--warning)', effect: 'confetti' };
  }
  if (count >= 3) {
